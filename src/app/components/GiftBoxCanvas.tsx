...
const GiftBoxCanvas: React.FC<GiftBoxCanvasProps> = ({
  ...
  const drawGiftBox = () => {
    ...
    if (!isModern) {
      ...
      // 蝴蝶结左半部分（增加渐变效果）
      const leftGradient = ctx.createLinearGradient(120, 60, 180, 100);
      leftGradient.addColorStop(0, frameColor);
      leftGradient.addColorStop(1, '#E8C547');
      ctx.fillStyle = leftGradient;
      ctx.beginPath();
      ctx.ellipse(160, 70, 45, 28, -Math.PI / 5, 0, 2 * Math.PI);
      ctx.fill();

      // 蝴蝶结右半部分
      const rightGradient = ctx.createLinearGradient(220, 60, 280, 100);
      rightGradient.addColorStop(0, frameColor);
      rightGradient.addColorStop(1, '#E8C547');
      ctx.fillStyle = rightGradient;
      ctx.beginPath();
      ctx.ellipse(240, 70, 45, 28, Math.PI / 5, 0, 2 * Math.PI);
      ctx.fill();

      // 蝴蝶结中心结（更立体）
      ctx.shadowBlur = 4;
      const centerGradient = ctx.createLinearGradient(190, 55, 210, 85);
      centerGradient.addColorStop(0, '#F1C40F');
      centerGradient.addColorStop(0.5, '#E8C547');
      centerGradient.addColorStop(1, '#D4AC0D');
      ctx.fillStyle = centerGradient;
      ctx.beginPath();
      ctx.ellipse(200, 70, 18, 25, 0, 0, 2 * Math.PI);
      ctx.fill();
      ...
      ctx.beginPath();
      ctx.moveTo(175, 55);
      ctx.quadraticCurveTo(200, 65, 225, 55);
      ctx.stroke();

      // 添加蝴蝶结高光
      ctx.fillStyle = 'rgba(255, 255, 255, 0.3)';
      ctx.beginPath();
      ctx.ellipse(185, 62, 15, 8, -Math.PI / 5, 0, 2 * Math.PI);
      ctx.fill();
      ctx.beginPath();
      ctx.ellipse(215, 62, 15, 8, Math.PI / 5, 0, 2 * Math.PI);
      ctx.fill();

      // 绘制垂直装饰带（更宽更立体）
      const ribbonGradient = ctx.createLinearGradient(185, 70, 215, 70);
      ribbonGradient.addColorStop(0, '#E8C547');
      ribbonGradient.addColorStop(0.5, frameColor);
      ribbonGradient.addColorStop(1, '#E8C547');
      ctx.fillStyle = ribbonGradient;
      ctx.fillRect(185, 70, 30, 30);
      
      // 垂直带的边框和装饰
      ctx.strokeStyle = '#D4AC0D';
      ctx.lineWidth = 2;
      ctx.strokeRect(185, 70, 30, 30);
      ...
      ctx.fillRect(width/2 - 20, 60, 40, 8);
      ctx.fillStyle = 'rgba(255, 255, 255, 0.8)';
      ctx.fillRect(width/2 - 15, 62, 30, 4);
    }
    ...
    if (!isModern) {
      ...
      ctx.fillText(textInfo.icon, width / 2, 115);
    }
