...
const Card: React.FC<CardProps> = ({ title, description, imageUrl, className, isLarge }) => {
    return (
        <div className={`relative bg-white rounded-extra-large shadow-md overflow-hidden transition-transform transition-shadow duration-500 ease-in-out transform hover:scale-105 hover:shadow-2xl ${className}`}>
            ...
            {/* 内容层 */}
            <div className={`absolute inset-0 flex flex-col justify-between p-6 pl-12  text-white transition-opacity duration-500 ease-in-out ${isLarge ? 'text-left' : 'items-center'}`}>
                {isLarge ? (
                    <>
                        {/* 顶部两行艺术化文字 */}
                        <div className="mb-4 space-y-4 mt-8">
                            <h3 className="text-3xl font-extrabold tracking-wide">{title}</h3>
                            <h3 className="text-3xl font-extrabold tracking-wide">{description}</h3>
                        </div>

                        {/* 底部左侧圆角按钮 */}
                        <div className="self-start w-56">
