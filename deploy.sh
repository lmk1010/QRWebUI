...
# 一键部署脚本 - QRWebUI到172.245.62.112服务器
# 使用方法: ./deploy.sh
...
DOCKER_IMAGE_NAME="qrwebui"
CONTAINER_NAME="qrwebui-app"
PORT="8100"

echo "🚀 开始部署 QRWebUI 到服务器 $SERVER_IP..."
...
# 2. 保存镜像为tar文件
echo "💾 保存Docker镜像..."
docker save $DOCKER_IMAGE_NAME > qrwebui.tar
...
# 3. 传输镜像到服务器
echo "📤 传输镜像到服务器..."
sshpass -p "$SERVER_PASSWORD" scp -o StrictHostKeyChecking=no qrwebui.tar $SERVER_USER@$SERVER_IP:/tmp/
